...
def estimate_lag_and_plot(
    ...
    mu_path = TONGUE_SCRIPTS_DIR / "normalising_vectors" / "JW13_4points_mu.npy"
    std_path = TONGUE_SCRIPTS_DIR / "normalising_vectors" / "JW13_4points_std.npy"
    ...
    for sid, dataset_id, dur in chosen:
        ...
        if not npy_path.exists():
            run(
                [
                    py,
                    str(TONGUE_SCRIPTS_DIR / "inversion" / "invert.py"),
                    "--wav",
                    str(wav_path),
                    "--out",
                    str(npy_path),
        ...
        run(
            [
                py,
                str(TONGUE_SCRIPTS_DIR / "evaluation" / "evaluate_vsr_ver.py"),
                "--videos",
                str(active_video),
                str(passive_video),
                "--ground-truth",
