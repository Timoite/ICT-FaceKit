...
from transformers import Wav2Vec2Model, Wav2Vec2Config, Wav2Vec2Processor, AutoProcessor, WavLMModel, AutoFeatureExtractor
...
class WavLMEncoderLayer(nn.Module):
    def __init__(self, config, has_relative_position_bias: bool = True):
        ...
        if self.config.finetune_method == "lora":
            self.feed_forward.intermediate_dense    = lora.Linear(config.hidden_size, config.intermediate_size, r=config.lora_rank,lora_alpha=config.lora_alpha,merge_weights=False)
            self.feed_forward.output_dense          = lora.Linear(config.intermediate_size, config.hidden_size, r=config.lora_rank,lora_alpha=config.lora_alpha,merge_weights=False)
            self.attention.k_proj                   = lora.Linear(config.hidden_size,config.hidden_size,r=config.lora_rank,lora_alpha=config.lora_alpha,merge_weights=False)
            self.attention.q_proj                   = lora.Linear(config.hidden_size,config.hidden_size,r=config.lora_rank,lora_alpha=config.lora_alpha,merge_weights=False)
            self.attention.v_proj                   = lora.Linear(config.hidden_size,config.hidden_size,r=config.lora_rank,lora_alpha=config.lora_alpha,merge_weights=False)
            self.attention.out_proj                   = lora.Linear(config.hidden_size,config.hidden_size,r=config.lora_rank,lora_alpha=config.lora_alpha,merge_weights=False)
